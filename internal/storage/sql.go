...
var (
	...
	CREATE_DISK_TABLE = `
		CREATE TABLE IF NOT EXISTS disk (
			...
			direct_mount_in_container TEXT DEFAULT "No",
			chunkserver_id TEXT DEFAULT "-",
			lastmodified_time DATE NOT NULL
		)
	`
	...
	// disk
	INSERT_DISK = `INSERT INTO disk(host, device, mount_point, format_percent, container_image_location,
		lastmodified_time) VALUES(?, ?, ?, ?, ?, datetime('now','localtime'))`

	SET_DISK = `UPDATE disk SET mount_point = ?, format_percent = ?, container_image_location = ?,
	...
	SET_DISK_CHUNKSERVER_ID = `UPDATE disk SET chunkserver_id = ?,
		lastmodified_time = datetime('now','localtime') WHERE host = ? AND mount_point = ?`
	...
	SELECT_DISK_BY_HOST_DEVICE = `SELECT * from disk WHERE host = ? AND device = ?`

	SELECT_DISK_BY_HOST_MOUNTPOINT = `SELECT * from disk WHERE host = ? AND mount_point = ?`
