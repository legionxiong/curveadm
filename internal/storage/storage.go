...
func (s *Storage) SetDisk(host, device, mount, containerImag string, format_percent int) error {
	disks, err := s.GetDisk(SELECT_DISK_BY_HOST_DEVICE, host, device)
	if err != nil {
		return err
	} else if len(disks) == 0 {
		return s.execSQL(INSERT_DISK, host, device, mount, format_percent, containerImag)
	}
...
func (s *Storage) UpdateDiskSize(host, device string, size int64) error {
...
func (s *Storage) GetDiskByMountPoint(host, mountPoint string) ([]Disk, error) {
	disks, err := s.GetDisk(SELECT_DISK_BY_HOST_MOUNTPOINT, host, mountPoint)
	if err != nil {
		return disks, err
	}
	if len(disks) == 0 {
		return disks, errno.ERR_DATABASE_EMPTY_QUERY_RESULT.
			F("The disk[host=%s, mount_point=%s] was not found in database.", host, mountPoint)
	}
...
func (s *Storage) CleanDiskChunkServerId(serviceId string) error {
	disks, err := s.GetDisk("service", serviceId)
	if err != nil {
		return err
	}

	for _, disk := range disks {
		if err := s.UpdateDiskChunkServerID(disk.Host, disk.MountPoint, "-"); err != nil {
			return err
		}
	}
	return nil
