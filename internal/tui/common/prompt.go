...
func PromptReplaceDisk(id, role, host, disk string) string {
	prompt := NewPrompt(color.YellowString(PROMPT_COMMON_WARNING) + DEFAULT_CONFIRM_PROMPT)
	prompt.data["warning"] = "WARNING: replace disk will stop service which may cause client IO hang"
	prompt.data["id"] = id
	prompt.data["rule"] = role
